...
DROP TABLE IF EXISTS ${db}_grid_weg_class;
CREATE TABLE ${db}_grid_weg_class AS
...
-- index
CREATE INDEX ${db}_grid_weg_class_idx ON ${db}_grid_weg_class (grid_gid);
VACUUM ANALYZE ${db}_grid_weg_class;
...
# -- 
# DROP TABLE IF EXISTS ssurgo_grid_weg_class;
# CREATE TABLE ssurgo_grid_weg_class AS
# SELECT DISTINCT ON (grid_gid) grid_gid, SUM(area_wt * (comppct_r::numeric/100.0)) as area_extent, weg
# FROM grid_mapunit 
...
# CREATE INDEX grid_weg_class_idx ON ssurgo_grid_weg_class (grid_gid);
# VACUUM ANALYZE ssurgo_grid_weg_class;
...
# -- get most extensive WEG
# -- 
# CREATE TABLE statsgo_grid_weg_class AS
# SELECT DISTINCT ON (grid_gid) grid_gid, SUM(area_wt * (comppct_r::numeric/100.0)) as area_extent, weg
