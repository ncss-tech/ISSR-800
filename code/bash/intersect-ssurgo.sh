...
echo "TRUNCATE conus_800m_grid.ssurgo_grid_mapunit;" | psql -U postgres ssurgo_combined

time cat chunk-IDs.txt | parallel --no-notice --joblog ssurgo-intersection.log --eta --progress  "echo \"
SET work_mem to '1GB' ;
INSERT INTO conus_800m_grid.ssurgo_grid_mapunit
...
JOIN
ssurgo.mapunit_poly ON ST_Intersects(ST_Transform(conus_800m_grid.grid.geom, 4326), mapunit_poly.wkb_geometry)
WHERE grid.chunk = '{}';
